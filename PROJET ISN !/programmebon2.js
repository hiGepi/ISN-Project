function main(){
    ...
    function background(){
		...
		if(tps > timemob){
			...
			timebonus += 8000;
			bonus.push( {x: Math.floor((canvas.width-10)*Math.random())+5, y: Math.floor((canvas.height-100)*Math.random())+50, size:9, yv:0, img:img_heal});  //Daniel
		}
        ...
        for( var b=0; b <= bonus.length-1; b++){                //Daniel
